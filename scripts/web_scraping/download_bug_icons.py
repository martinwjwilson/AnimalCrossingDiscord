...
all_links = []
table_rows = soup.find_all("tr")[4:84] # get all table rows with the fish info
...
for row in all_rows:
    ...
    print(row[0])
    print("\n")
    c.execute(f"""UPDATE bugs SET image = '{all_links[i]}' WHERE name = '{row[0]}'""") # prepare SQL statement
    i += 1
